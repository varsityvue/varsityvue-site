...
export function parseGameStatsDraft(input: string): GameStatsImportResult {
  ...
  try {
    ...
    return { ok: false, errors: ["The pasted content is not valid JSON."] };
  }

  if (!isRecord(parsed)) {
    return { ok: false, errors: ["The import must be a single game-stat object."] };
  }
  ...
  if (!Number.isInteger(parsed.season)) {
    errors.push("season must be an integer year.");
  }

  if (parsed.sourceStatus !== "verified") {
    errors.push('sourceStatus must be "verified" before review can be approved.');
  }
...
export function resolveKnownPlayerIds(stats: GameStats) {
  ...
  for (const schoolSlug of schoolSlugs) {
    for (const profile of getSchoolPlayerProfiles(schoolSlug, stats.season)) {
      profileLookup.set(`${schoolSlug}:${profile.name.trim().toLowerCase()}`, profile.playerId);
    }
  }
  ...
  function resolveLine<T extends { player: string; schoolSlug: string; playerId?: string }>(line: T): T {
    ...
    if (rosterId) {
      notices.push(`Matched ${line.player} to roster playerId ${rosterId}.`);
      return { ...line, playerId: rosterId };
    }
  ...
  return {
    stats: {
      ...stats,
      rushing: stats.rushing.map(resolveLine),
      passing: stats.passing.map(resolveLine),
      receiving: stats.receiving.map(resolveLine),
    },
    notices,
  };
}

export function formatGameStatsForDataFile(stats: GameStats) {
