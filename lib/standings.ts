...
type RecordOverride = Pick<Standing, "overallWins" | "overallLosses" | "districtWins" | "districtLosses">;
...
type StandingGame = Game;

function emptyStanding(schoolSlug: string, team: string): Standing {
  return { schoolSlug, team, districtWins: 0, districtLosses: 0, overallWins: 0, overallLosses: 0, pointsFor: 0, pointsAgainst: 0, overallRecordKnown: false };
}
...
function applyVerifiedStandingOverride(standing: Standing, games: StandingGame[]) {
  ...
  if (typeof override.pointsFor === "number" && typeof override.pointsAgainst === "number" && typeof throughWeek === "number") {
    ...
    for (const game of games) {
      if (typeof game.week === "number" && game.week > throughWeek) applyGameToStanding(standing, standing.schoolSlug, game);
    }
    return;
  }
...
function hasDistrictResults(standings: Standing[]) {
  return standings.some((standing) => standing.districtWins > 0 || standing.districtLosses > 0);
}

function sortStandings(standings: Standing[]) {
  if (!hasDistrictResults(standings)) return [...standings].sort((a, b) => a.team.localeCompare(b.team));
...
function addScheduledDistrictOpponents(standingsMap: Map<string, Standing>, districtSchoolSlugs: Set<string>, games: StandingGame[]) {
  for (const game of games) {
    if (!game.districtGame) continue;
    const homeIsDistrictSchool = Boolean(game.homeSchoolSlug && districtSchoolSlugs.has(game.homeSchoolSlug));
    const awayIsDistrictSchool = Boolean(game.awaySchoolSlug && districtSchoolSlugs.has(game.awaySchoolSlug));
    if (homeIsDistrictSchool && game.awaySchoolSlug && game.awayTeam && !standingsMap.has(game.awaySchoolSlug)) {
      standingsMap.set(game.awaySchoolSlug, emptyStanding(game.awaySchoolSlug, game.awayTeam));
    }
    if (awayIsDistrictSchool && game.homeSchoolSlug && game.homeTeam && !standingsMap.has(game.homeSchoolSlug)) {
      standingsMap.set(game.homeSchoolSlug, emptyStanding(game.homeSchoolSlug, game.homeTeam));
    }
  }
}
...
function buildStandingsForDistrict(districtId: string, games: StandingGame[]): Standing[] {
  ...
  addScheduledDistrictOpponents(standingsMap, new Set(districtSchools.map((school) => school.slug)), games);
  games.forEach((game) => standingsMap.forEach((standing, schoolSlug) => applyGameToStanding(standing, schoolSlug, game)));
  standingsMap.forEach((standing) => applyVerifiedStandingOverride(standing, games));
  return sortStandings(Array.from(standingsMap.values()));
}

function buildStandaloneStanding(slug: string, games: StandingGame[]): Standing | undefined {
  const school = getSchoolBySlug(slug);
  const matchingGames = games.filter((game) => game.homeSchoolSlug === slug || game.awaySchoolSlug === slug);
  const override = verifiedStandingOverrides[slug];
  if (!school && matchingGames.length === 0 && !override) return undefined;
  const teamName = school?.name ?? matchingGames.find((game) => game.homeSchoolSlug === slug)?.homeTeam ?? matchingGames.find((game) => game.awaySchoolSlug === slug)?.awayTeam ?? slug;
  const standing = emptyStanding(slug, teamName);
  matchingGames.forEach((game) => applyGameToStanding(standing, slug, game));
  applyVerifiedStandingOverride(standing, games);
  return standing;
}
...
export function getStandingsForSchool(slug: string): Standing[] {
  const school = getSchoolBySlug(slug);
  if (!school) {
    const standing = buildStandaloneStanding(slug, staticGames);
    return standing ? [standing] : [];
  }
  return buildStandingsForDistrict(school.districtId, staticGames);
}

export function getStandingsForDistrictId(districtId: string): Standing[] {
  return buildStandingsForDistrict(districtId, staticGames);
}

export function getStandingsForDistrictIdFromGames(districtId: string, games: Game[]): Standing[] {
  return buildStandingsForDistrict(districtId, games);
}

export function getStandingForSchool(slug: string): Standing | undefined {
  const school = getSchoolBySlug(slug);
  if (school) {
    const districtStanding = getStandingsForDistrictId(school.districtId).find((standing) => standing.schoolSlug === slug);
