...
import { getSchoolSeasonCategoryCompleteness, type AggregateStatCompleteness } from "@/lib/stat-completeness";
...
export type PlayerSeasonStats = {
  ...
  completeness: {
    ...
    passing: AggregateStatCompleteness;
    receiving: AggregateStatCompleteness;
  };
};

export type RushingLeaderboardEntry = Pick<PlayerSeasonStats, "playerId" | "player" | "schoolSlug" | "season" | "gamesRecorded"> & { rushing: PlayerSeasonStats["rushing"]; completeness: AggregateStatCompleteness };
export type PassingLeaderboardEntry = Pick<PlayerSeasonStats, "playerId" | "player" | "schoolSlug" | "season" | "gamesRecorded"> & { passing: PlayerSeasonStats["passing"]; completeness: AggregateStatCompleteness };
export type ReceivingLeaderboardEntry = Pick<PlayerSeasonStats, "playerId" | "player" | "schoolSlug" | "season" | "gamesRecorded"> & { receiving: PlayerSeasonStats["receiving"]; completeness: AggregateStatCompleteness };
...
function emptyPlayerSeasonStats({ playerId, player, schoolSlug, season }: { playerId: string; player: string; schoolSlug: string; season: number }): PlayerSeasonStats {
  return {
    ...
    completeness: {
      rushing: getSchoolSeasonCategoryCompleteness(schoolSlug, season, "rushing"),
      passing: getSchoolSeasonCategoryCompleteness(schoolSlug, season, "passing"),
      receiving: getSchoolSeasonCategoryCompleteness(schoolSlug, season, "receiving"),
    },
  };
}
...
export function getRushingLeaders({ season = 2026, schoolSlug, districtId, minAttempts = 0, sortBy = "yards" }: { season?: number; schoolSlug?: string; districtId?: string; minAttempts?: number; sortBy?: "yards" | "touchdowns" | "yardsPerCarry" } = {}): RushingLeaderboardEntry[] {
  return getPlayerSeasonStats(season).filter((player) => matchesScope(player, schoolSlug, districtId) && player.rushing.attempts >= minAttempts).sort((a, b) => { const primary = compareOptionalStatsDescending(a.rushing[sortBy], b.rushing[sortBy]); return primary !== 0 ? primary : b.rushing.yards - a.rushing.yards; }).map(({ playerId, player, schoolSlug: slug, season: playerSeason, gamesRecorded, rushing, completeness }) => ({ playerId, player, schoolSlug: slug, season: playerSeason, gamesRecorded, rushing, completeness: completeness.rushing }));
}
...
export function getReceivingLeaders({ season = 2026, schoolSlug, districtId, minReceptions = 0, sortBy = "yards" }: { season?: number; schoolSlug?: string; districtId?: string; minReceptions?: number; sortBy?: "yards" | "touchdowns" | "receptions" | "yardsPerReception" } = {}): ReceivingLeaderboardEntry[] {
  return getPlayerSeasonStats(season).filter((player) => matchesScope(player, schoolSlug, districtId) && player.receiving.receptions >= minReceptions).sort((a, b) => { const primary = compareOptionalStatsDescending(a.receiving[sortBy], b.receiving[sortBy]); return primary !== 0 ? primary : b.receiving.yards - a.receiving.yards; }).map(({ playerId, player, schoolSlug: slug, season: playerSeason, gamesRecorded, receiving, completeness }) => ({ playerId, player, schoolSlug: slug, season: playerSeason, gamesRecorded, receiving, completeness: completeness.receiving }));
}
