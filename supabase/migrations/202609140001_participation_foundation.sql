...
create type public.score_submission_status as enum (
  'pending',
  'approved',
  'rejected',
  'superseded'
...
create type public.score_submission_event_type as enum (
  'submitted',
  'approved',
  'rejected',
  'superseded',
...
create type public.pickem_week_status as enum (
  'draft',
  'open',
  'locked',
  'graded'
...
-- ---------------------------------------------------------------------------
-- CANONICAL DYNAMIC GAME STATE
-- ---------------------------------------------------------------------------
-- game_id references the stable IDs already stored in data/games.ts. We do not
-- duplicate the full static schedule in Postgres yet. This table is the bridge
-- that lets approved community updates override dynamic state on the site.

create table public.game_state (
  game_id text primary key,
  status text not null default 'upcoming'
    check (status in ('scheduled', 'upcoming', 'live', 'final', 'postponed', 'cancelled')),
  ...
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint game_state_scores_together
...
-- ---------------------------------------------------------------------------
-- COMMUNITY SCORE SUBMISSIONS + AUDIT TRAIL
-- ---------------------------------------------------------------------------
...
create table public.score_submissions (
  ...
  game_status text not null default 'live'
    check (game_status in ('upcoming', 'live', 'final', 'postponed', 'cancelled')),
  period text,
  clock text,
  source_note text,
  source_url text,
...
alter table public.game_state
  add constraint game_state_source_submission_fkey
  foreign key (source_submission_id)
  references public.score_submissions(id)
  on delete set null;

create index score_submissions_game_id_idx
  on public.score_submissions(game_id, created_at desc);

create index score_submissions_status_idx
  on public.score_submissions(status, created_at asc);

create index score_submissions_submitter_idx
  on public.score_submissions(submitted_by, created_at desc);
...
create index score_submission_events_submission_idx
  on public.score_submission_events(submission_id, created_at asc);
...
create trigger score_submission_created_event
after insert on public.score_submissions
for each row execute procedure public.log_score_submission_created();
...
create trigger pickem_weeks_set_updated_at
...
create index pickem_games_week_idx
  on public.pickem_games(week_id, sort_order);
...
create index pickem_picks_user_idx
  on public.pickem_picks(user_id, submitted_at desc);

create trigger pickem_picks_set_updated_at
before update on public.pickem_picks
for each row execute procedure public.set_updated_at();

-- Standings are derived instead of manually maintained, preventing leaderboard
-- totals from drifting away from the underlying picks.
...
select
  ...
  case
    when count(*) filter (where p.is_correct is not null) = 0 then 0::numeric
    else round(
...
-- Profiles: public identity is readable; users can maintain their own profile.
...
-- Roles: a user can see their own roles; moderators/admins can inspect all.
create policy "Users can read own roles"
on public.user_roles for select
to authenticated
using (user_id = auth.uid() or public.can_moderate_scores());

-- Canonical game state is public; only moderators/admins mutate it directly.
...
to authenticated
with check (public.can_moderate_scores());

create policy "Moderators can update game state"
on public.game_state for update
to authenticated
using (public.can_moderate_scores())
with check (public.can_moderate_scores());

-- Score submissions: members can submit and see their own; moderation staff
-- can review all. Submitters cannot approve their own records through RLS.
...
to authenticated
using (submitted_by = auth.uid() or public.can_moderate_scores());
...
using (
  exists (
    ...
    where s.id = submission_id
      and (s.submitted_by = auth.uid() or public.can_moderate_scores())
  )
);

-- Pick 'Em configuration is public once created; moderation/admin controls the
-- selected games. Users can only insert/update their own picks. Lock-time
-- enforcement is also performed server-side in the application layer.
create policy "Pickem weeks are publicly readable"
on public.pickem_weeks for select
using (true);

create policy "Pickem games are publicly readable"
on public.pickem_games for select
using (true);

create policy "Moderators manage pickem weeks"
on public.pickem_weeks for all
to authenticated
using (public.can_moderate_scores())
with check (public.can_moderate_scores());

create policy "Moderators manage pickem games"
on public.pickem_games for all
to authenticated
using (public.can_moderate_scores())
with check (public.can_moderate_scores());
...
create policy "Users can update own ungraded picks"
on public.pickem_picks for update
to authenticated
using (user_id = auth.uid() and is_correct is null)
with check (user_id = auth.uid() and is_correct is null);

-- ---------------------------------------------------------------------------
-- NOTES FOR APPLICATION INTEGRATION
-- ---------------------------------------------------------------------------
-- 1. data/games.ts remains the schedule/source data for now.
-- 2. The site should merge public.game_state over the matching static game ID.
-- 3. Score approval should run through a trusted server action/route that:
--      a) verifies moderator/admin permission,
--      b) marks the submission approved,
--      c) writes a score_submission_events audit record,
--      d) upserts public.game_state,
--      e) supersedes conflicting older pending submissions when appropriate.
-- 4. Pick 'Em writes must be rejected server-side when now() >= lock_at even
--    though the UI also disables the choice.
-- 5. Service-role credentials must never be exposed to browser code.
