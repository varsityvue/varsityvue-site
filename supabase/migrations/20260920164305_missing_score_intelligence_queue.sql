...
create policy "Moderators read missing score intelligence" on public.missing_score_intelligence for select to authenticated using ((select private.can_moderate_scores()));
create policy "Moderators update missing score intelligence" on public.missing_score_intelligence for update to authenticated using ((select private.can_moderate_scores())) with check ((select private.can_moderate_scores()));
create policy "Moderators read missing score evidence" on public.missing_score_evidence for select to authenticated using ((select private.can_moderate_scores()));
create policy "Moderators add missing score evidence" on public.missing_score_evidence for insert to authenticated with check ((select private.can_moderate_scores()) and captured_by = (select auth.uid()));
create or replace function public.sync_missing_score_intelligence(worker_secret text, candidates jsonb, resolved_game_ids text[]) returns jsonb language plpgsql security definer set search_path = '' as $$
...
begin
  ...
  select item->>'game_id', nullif(item->>'week','')::integer, (item->>'kickoff')::timestamptz, left(item->>'away_team', 120), left(item->>'home_team', 120), nullif(item->>'away_school_slug',''), nullif(item->>'home_school_slug','')
  from jsonb_array_elements(candidates) item
  where item->>'game_id' ~ '^[a-z0-9-]{3,200}$' and char_length(item->>'away_team') between 1 and 120 and char_length(item->>'home_team') between 1 and 120
  on conflict (game_id) do update set last_checked_at = now(), kickoff = excluded.kickoff, week = excluded.week, away_team = excluded.away_team, home_team = excluded.home_team, away_school_slug = excluded.away_school_slug, home_school_slug = excluded.home_school_slug, updated_at = now()
...
create or replace function private.resolve_missing_score_intelligence() returns trigger language plpgsql security definer set search_path = '' as $$
begin
  if new.verified and new.status in ('final', 'cancelled', 'postponed') then
    update public.missing_score_intelligence set status = 'resolved', resolved_at = now(), resolution = new.status, updated_at = now()
    where game_id = new.game_id and status = 'open';
  end if;
  return new;
end; $$;
create trigger resolve_missing_score_intelligence_after_game_state after insert or update on public.game_state for each row execute function private.resolve_missing_score_intelligence();
