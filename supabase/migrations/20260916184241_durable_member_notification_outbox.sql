...
$$;
revoke all on function private.member_notification_worker_authorized(text) from public, anon, authenticated;
create or replace function public.claim_member_notification(worker_secret text)
returns table (event_id uuid, user_id uuid, email text, display_name text, attempt_count integer)
...
begin
  ...
  return query select claimed.id, claimed.user_id, claimed.email, claimed.display_name, claimed.attempt_count;
end;
$$;
create or replace function public.complete_member_notification(
  worker_secret text, target_event_id uuid, resend_message_id text
)
...
create or replace function public.fail_member_notification(
  worker_secret text, target_event_id uuid, failure_detail text
)
returns boolean language plpgsql security definer set search_path = ''
as $$
begin
  ...
  set status = 'failed',
      ...
      updated_at = now()
  where event.id = target_event_id and event.status = 'processing';
  return found;
end;
$$;
revoke all on function public.claim_member_notification(text) from public, authenticated, service_role;
revoke all on function public.complete_member_notification(text, uuid, text) from public, authenticated, service_role;
revoke all on function public.fail_member_notification(text, uuid, text) from public, authenticated, service_role;
