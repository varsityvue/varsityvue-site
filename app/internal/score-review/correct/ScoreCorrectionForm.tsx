...
export default function ScoreCorrectionForm({ gameId, awayName, homeName, state, isAdmin }: Props) {
  ...
  return <form action={correctScore} className="mt-6 grid max-w-xl gap-4 rounded-2xl border border-white/15 bg-white/[0.04] p-4 sm:p-6">
    ...
    <p className="text-sm font-bold text-white/80">{priorFinal ? "Correcting a finalized result" : state.away_score !== null ? "Correcting an entered live score" : "Entering the first score"}</p>
    ...
    {!priorFinal && status === "final" && <p className="rounded-xl border border-amber-300/30 bg-amber-300/10 p-3 text-sm text-amber-100">Saving this as FINAL records the first verified final and may send the ordinary FINAL notification.</p>}
