      ...
      <div className="grid gap-2 sm:gap-4">{games.map((game) => {
        ...
        return <div key={game.id} className="group relative overflow-hidden rounded-[1rem] border border-white/10 bg-white/[0.04] p-3 shadow-xl transition hover:bg-white/[0.07] sm:rounded-[1.5rem] sm:p-6" style={{ boxShadow: `0 14px 38px ${school.colors.primary}10` }}>
          ...
          <div className="relative"><div className="flex flex-wrap gap-1 sm:gap-2"><Badge label={getGameTypeLabel(game)} /><Badge label={getGameStatusLabel(game, todayKey)} />{!isBye && <Badge label={locationLabel} />}{game.districtGame && <Badge label="District" />}{game.specialEvent && <Badge label={game.specialEvent} />}{statAvailability && statAvailability.status !== "verified" && <StatBadge label={getStatAvailabilityLabel(statAvailability.status)} />}</div>
            ...
            {!isBye && <div className="mt-2.5 flex flex-wrap items-center gap-2 sm:mt-5 sm:gap-3">{mapUrl && <a href={mapUrl} target="_blank" rel="noopener noreferrer" className="text-[8px] font-black uppercase tracking-[0.09em] text-white/40 transition hover:text-white sm:rounded-full sm:border sm:border-white/10 sm:bg-white/10 sm:px-4 sm:py-2 sm:text-xs sm:tracking-[0.14em] sm:text-white/65">Map →</a>}{reportScoreLabel && <Link href={`/report-score?game=${encodeURIComponent(game.id)}`} className="rounded-full border border-[var(--vv-accent)]/30 bg-[var(--vv-accent)]/10 px-3 py-1.5 text-[8px] font-black uppercase tracking-[0.09em] text-white transition hover:bg-[var(--vv-accent)]/20 sm:px-4 sm:py-2 sm:text-xs sm:tracking-[0.14em]">{reportScoreLabel} →</Link>}<Link href={`/games/${game.id}`} className="ml-auto text-[8px] font-black uppercase tracking-[0.09em] text-white/70 transition hover:text-white sm:ml-0 sm:rounded-full sm:border sm:border-white/10 sm:bg-white/[0.07] sm:px-4 sm:py-2 sm:text-xs sm:tracking-[0.14em] sm:text-white/80">Game Center →</Link></div>}
          </div>
        </div>;
      })}</div>}
    </div></section>
