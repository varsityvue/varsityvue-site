...
function LeaderboardSection({ id, title, note, headers, rows }: { id: string; title: string; note: string; headers: string[]; rows: (string | number | ReactNode)[][] }) {
  return <section id={id} className="scroll-mt-24 overflow-hidden rounded-[1.75rem] border border-white/10 bg-white/[0.045] shadow-2xl"><div className="h-1.5 bg-[#8B1020]" /><div className="p-5 sm:p-6 md:p-7"><div className="flex flex-wrap items-end justify-between gap-4"><div><p className="text-xs font-black uppercase tracking-[0.28em] text-white/40">District View</p><h2 className="mt-2 text-3xl font-black">{title}</h2></div><p className="text-xs font-black uppercase tracking-[0.14em] text-white/30">{note}</p></div>{rows.length ? <>
  ...
  </> : <div className="mt-6 rounded-2xl border border-white/10 bg-black/30 p-6 text-sm text-white/40">No verified statistics are available for this category yet.</div>}</div></section>;
}
