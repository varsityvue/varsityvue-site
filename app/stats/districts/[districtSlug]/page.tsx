...
function LeaderboardSection({ id, title, note, headers, rows }: { id: string; title: string; note: string; headers: string[]; rows: (string | number | ReactNode)[][] }) {
  ...
  return <section id={id} className="scroll-mt-24 overflow-hidden rounded-[1.4rem] border border-white/10 bg-white/[0.045] shadow-2xl sm:rounded-[1.75rem]"><div className="h-1 bg-[#8B1020] sm:h-1.5" /><div className="p-4 sm:p-6 md:p-7"><div className="flex flex-wrap items-end justify-between gap-3 sm:gap-4"><div><p className="text-[10px] font-black uppercase tracking-[0.2em] text-white/40 sm:text-xs sm:tracking-[0.28em]">District View</p><h2 className="mt-1.5 text-2xl font-black sm:mt-2 sm:text-3xl">{title}</h2></div><p className="text-[9px] font-black uppercase tracking-[0.1em] text-white/30 sm:text-xs sm:tracking-[0.14em]">{note}</p></div><p className="mt-2 text-[10px] leading-4 text-white/35 sm:text-xs sm:leading-5">Ordinal ranks are shown only when every listed total is explicitly complete. Partial and unclassified totals remain visible and ordered without claiming a definitive district rank.</p>{rows.length ? <>
