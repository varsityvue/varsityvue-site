...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    description: `${district.name} 2026 rushing, passing, receiving, and efficiency leaders from verified VarsityVue game statistics.`,
  };
}
    ...
    <main className="min-h-screen bg-[#050505] text-white">
      <section className="border-b border-white/10 bg-[radial-gradient(circle_at_top_left,rgba(139,16,32,0.38),transparent_34%),linear-gradient(120deg,#050505_0%,#090909_50%,#000_100%)] px-4 py-10 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-7xl">
          ...
          <p className="mt-6 text-xs font-black uppercase tracking-[0.28em] text-[#F4EBDD]/60">2026 District Leaders</p>
          ...
          <p className="mt-4 max-w-3xl text-base leading-7 text-white/55">
            Rankings are calculated from verified game statistics currently loaded into VarsityVue for programs in this district. Totals become more complete as additional weekly reports are added.
          </p>
...
function schoolLink(schoolSlug: string) {
  ...
  return <Link href={`/schools/${schoolSlug}`} className="font-black text-white/75 transition hover:text-white">{school?.name ?? schoolSlug}</Link>;
}

function LeaderboardSection({ id, title, note, headers, rows }: {
  id: string;
  title: string;
  note: string;
  headers: string[];
    ...
    <section id={id} className="scroll-mt-24 overflow-hidden rounded-[1.75rem] border border-white/10 bg-white/[0.045] shadow-2xl">
      ...
      <div className="p-6 md:p-7">
        <div className="flex flex-wrap items-end justify-between gap-4">
          <div>
            <p className="text-xs font-black uppercase tracking-[0.28em] text-white/40">District Leaders</p>
        ...
        {rows.length ? (
          <div className="mt-6 overflow-x-auto rounded-2xl border border-white/10 bg-black/30">
            <table className="w-full min-w-[760px] text-sm">
              <thead className="border-b border-white/10 text-[10px] font-black uppercase tracking-[0.12em] text-white/35"><tr>{headers.map((header) => <th key={header} className="px-4 py-3 text-left">{header}</th>)}</tr></thead>
              <tbody>{rows.map((row, rowIndex) => <tr key={rowIndex} className="border-t border-white/5 first:border-0">{row.map((value, cellIndex) => <td key={cellIndex} className={`px-4 py-3 ${cellIndex === 0 ? "font-black text-white/35" : "text-white/75"}`}>{value}</td>)}</tr>)}</tbody>
            </table>
          </div>
        ) : (
