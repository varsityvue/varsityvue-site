  ...
  return (
    <main className="min-h-screen bg-[#050505] text-white">
      ...
      <section className="px-4 py-5 sm:px-6 sm:py-8 lg:px-8"><div className="mx-auto max-w-7xl space-y-5 sm:space-y-8">
        <LeaderboardSection id="rushing" eyebrow="Rushing Leaders" title="Rushing yards" note="Minimum 1 carry · Ordered by verified values on file" headers={["RK", "Player", "School", "G", "CAR", "YDS", "TD", "YPC", "Coverage"]} rows={rushing.map((entry, index) => [index + 1, playerLink(entry.playerId, entry.player), schoolLink(entry.schoolSlug), entry.gamesRecorded, entry.rushing.attempts, entry.rushing.yards, formatTouchdownCount(entry.rushing.touchdowns), entry.rushing.yardsPerCarry, entry.completeness.status === "complete" || entry.completeness.status === "partial" ? <StatCompletenessBadge key="coverage" completeness={entry.completeness} compact /> : null])} />
        <LeaderboardSection id="passing" eyebrow="Passing Leaders" title="Passing yards" note="Minimum 1 attempt · Ordered by verified values on file" headers={["RK", "Player", "School", "G", "CMP/ATT", "YDS", "TD", "INT", "CMP%", "Coverage"]} rows={passing.map((entry, index) => [index + 1, playerLink(entry.playerId, entry.player), schoolLink(entry.schoolSlug), entry.gamesRecorded, `${entry.passing.completions}/${entry.passing.attempts}`, entry.passing.yards, formatTouchdownCount(entry.passing.touchdowns), entry.passing.interceptions, `${entry.passing.completionPercentage}%`, entry.completeness.status === "complete" || entry.completeness.status === "partial" ? <StatCompletenessBadge key="coverage" completeness={entry.completeness} compact /> : null])} />
        <LeaderboardSection id="receiving" eyebrow="Receiving Leaders" title="Receiving yards" note="Minimum 1 reception · Ordered by verified values on file" headers={["RK", "Player", "School", "G", "REC", "YDS", "TD", "YPR", "Coverage"]} rows={receiving.map((entry, index) => [index + 1, playerLink(entry.playerId, entry.player), schoolLink(entry.schoolSlug), entry.gamesRecorded, entry.receiving.receptions, entry.receiving.yards, formatTouchdownCount(entry.receiving.touchdowns), entry.receiving.yardsPerReception, entry.completeness.status === "complete" || entry.completeness.status === "partial" ? <StatCompletenessBadge key="coverage" completeness={entry.completeness} compact /> : null])} />
        <LeaderboardSection id="ypc" eyebrow="Efficiency Leaders" title="Yards per carry" note="Minimum 5 carries · Ordered by verified values on file" headers={["RK", "Player", "School", "G", "CAR", "YDS", "YPC", "Coverage"]} rows={ypc.map((entry, index) => [index + 1, playerLink(entry.playerId, entry.player), schoolLink(entry.schoolSlug), entry.gamesRecorded, entry.rushing.attempts, entry.rushing.yards, entry.rushing.yardsPerCarry, entry.completeness.status === "complete" || entry.completeness.status === "partial" ? <StatCompletenessBadge key="coverage" completeness={entry.completeness} compact /> : null])} />
      </div></section>
    </main>
  );
}
...
function LeaderboardSection({ id, eyebrow, title, note, headers, rows }: { id: string; eyebrow: string; title: string; note: string; headers: string[]; rows: (string | number | ReactNode)[][] }) {
  const mobilePreview = rows.slice(0, 3);
  const mobileRemainder = rows.slice(3);
  return <section id={id} className="scroll-mt-24 overflow-hidden rounded-[1.4rem] border border-white/10 bg-white/[0.045] shadow-2xl sm:rounded-[1.75rem]"><div className="h-1 bg-[#8B1020] sm:h-1.5" /><div className="p-4 sm:p-6 md:p-7"><div className="flex flex-wrap items-end justify-between gap-2.5 sm:gap-4"><div><p className="text-[9px] font-black uppercase tracking-[0.2em] text-white/40 sm:text-xs sm:tracking-[0.28em]">{eyebrow}</p><h2 className="mt-1.5 text-2xl font-black sm:mt-2 sm:text-3xl">{title}</h2></div><p className="w-full text-[9px] font-black uppercase tracking-[0.1em] text-white/30 sm:w-auto sm:text-xs sm:tracking-[0.14em]">{note}</p></div>
    {rows.length ? <>
