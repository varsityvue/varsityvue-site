...
export default function StatsPage() {
  ...
  return (
    <main className="min-h-screen bg-[#050505] text-white">
      ...
      <section className="border-b border-white/10 px-4 py-6 sm:px-6 lg:px-8"><div className="mx-auto max-w-7xl"><div className="flex flex-wrap items-center justify-between gap-4"><div><p className="text-xs font-black uppercase tracking-[0.22em] text-white/35">District Views</p><p className="mt-2 text-sm text-white/50">Compare available verified leaders within a specific UIL district.</p></div><div className="flex flex-wrap gap-2">{districtsWithStats.length > 0 ? districtsWithStats.map((district) => <Link key={district.id} href={`/stats/districts/${district.slug}`} className="rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-xs font-black uppercase tracking-[0.12em] text-white/65 transition hover:bg-white/10 hover:text-white">{district.name}</Link>) : <span className="text-sm text-white/35">District views will appear as verified stats are added.</span>}</div></div></div></section>

      <section className="px-4 py-8 sm:px-6 lg:px-8"><div className="mx-auto max-w-7xl space-y-8">
        <LeaderboardSection id="rushing" eyebrow="Rushing Leaders" title="Rushing yards" note="Minimum 1 carry · Verified stats on file" headers={["RK", "Player", "School", "G", "CAR", "YDS", "TD", "YPC"]} rows={rushing.map((entry, index) => [index + 1, playerLink(entry.playerId, entry.player), schoolLink(entry.schoolSlug), entry.gamesRecorded, entry.rushing.attempts, entry.rushing.yards, entry.rushing.touchdowns, entry.rushing.yardsPerCarry])} />
        <LeaderboardSection id="passing" eyebrow="Passing Leaders" title="Passing yards" note="Minimum 1 attempt · Verified stats on file" headers={["RK", "Player", "School", "G", "CMP/ATT", "YDS", "TD", "INT", "CMP%"]} rows={passing.map((entry, index) => [index + 1, playerLink(entry.playerId, entry.player), schoolLink(entry.schoolSlug), entry.gamesRecorded, `${entry.passing.completions}/${entry.passing.attempts}`, entry.passing.yards, entry.passing.touchdowns, entry.passing.interceptions, `${entry.passing.completionPercentage}%`])} />
        <LeaderboardSection id="receiving" eyebrow="Receiving Leaders" title="Receiving yards" note="Minimum 1 reception · Verified stats on file" headers={["RK", "Player", "School", "G", "REC", "YDS", "TD", "YPR"]} rows={receiving.map((entry, index) => [index + 1, playerLink(entry.playerId, entry.player), schoolLink(entry.schoolSlug), entry.gamesRecorded, entry.receiving.receptions, entry.receiving.yards, entry.receiving.touchdowns, entry.receiving.yardsPerReception])} />
...
function LeaderboardSection({ id, eyebrow, title, note, headers, rows }: { id: string; eyebrow: string; title: string; note: string; headers: string[]; rows: (string | number | ReactNode)[][] }) {
  return <section id={id} className="scroll-mt-24 overflow-hidden rounded-[1.75rem] border border-white/10 bg-white/[0.045] shadow-2xl"><div className="h-1.5 bg-[#8B1020]" /><div className="p-6 md:p-7"><div className="flex flex-wrap items-end justify-between gap-4"><div><p className="text-xs font-black uppercase tracking-[0.28em] text-white/40">{eyebrow}</p><h2 className="mt-2 text-3xl font-black">{title}</h2></div><p className="text-xs font-black uppercase tracking-[0.14em] text-white/30">{note}</p></div>
