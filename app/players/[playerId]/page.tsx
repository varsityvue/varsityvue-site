  ...
  return (
    <main className="min-h-screen bg-[#050505] text-white">
      ...
      <section className="px-4 py-5 sm:px-6 sm:py-8 lg:px-8"><div className="mx-auto max-w-7xl space-y-5 sm:space-y-8">
        ...
        {hasStats && <section className="overflow-hidden rounded-[1.5rem] border border-white/10 bg-white/[0.045] shadow-2xl sm:rounded-[1.75rem]"><div className="h-1" style={{ backgroundColor: primary }} /><div className="p-4 sm:p-6 md:p-7"><p className="text-[9px] font-black uppercase tracking-[0.2em] text-white/40 sm:text-xs sm:tracking-[0.28em]">Ordered From Stats On File</p><h2 className="mt-1.5 text-2xl font-black sm:mt-2 sm:text-3xl">Current VarsityVue comparisons</h2><div className="mt-4 grid grid-cols-2 gap-2 sm:mt-6 sm:gap-4 lg:grid-cols-4"><RankCard label="Rushing" rank={rushingRank} active={player.rushing.attempts > 0} /><RankCard label="District Rushing" rank={districtRushingRank} active={player.rushing.attempts > 0} /><RankCard label="Passing" rank={passingRank} active={player.passing.attempts > 0} /><RankCard label="Receiving" rank={receivingRank} active={player.receiving.receptions > 0} /></div><p className="mt-4 text-[10px] leading-4 text-white/35 sm:mt-5 sm:text-xs sm:leading-5">Verified totals remain ordered for discovery, but ordinal ranks appear only when every listed total in that category is explicitly complete. Partial and unclassified totals are not presented as definitive rankings.</p></div></section>}
