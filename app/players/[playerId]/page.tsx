  ...
  return (
    <main className="min-h-screen bg-[#050505] text-white">
      ...
      <section className="px-4 py-5 sm:px-6 sm:py-8 lg:px-8"><div className="mx-auto max-w-7xl space-y-5 sm:space-y-8">
        ...
        {hasStats ? <div className="grid grid-cols-[repeat(auto-fit,minmax(220px,1fr))] gap-2.5 sm:gap-4">
          {hasRushing ? <StatCard label="Rushing" completeness={player.completeness.rushing} headline={`${player.rushing.yards.toLocaleString()} yards`} details={`${player.rushing.attempts} carries · ${formatTouchdownDetail(player.rushing.touchdowns)} · ${player.rushing.yardsPerCarry} YPC`} /> : null}
          {hasPassing ? <StatCard label="Passing" completeness={player.completeness.passing} headline={`${player.passing.yards.toLocaleString()} yards`} details={`${player.passing.completions}/${player.passing.attempts} · ${formatTouchdownDetail(player.passing.touchdowns)} · ${player.passing.interceptions} INT · ${player.passing.completionPercentage}%`} /> : null}
          {hasReceiving ? <StatCard label="Receiving" completeness={player.completeness.receiving} headline={`${player.receiving.yards.toLocaleString()} yards`} details={`${player.receiving.receptions} receptions · ${formatTouchdownDetail(player.receiving.touchdowns)} · ${player.receiving.yardsPerReception} YPR`} /> : null}
        </div> : null}

        {hasStats && <section className="overflow-hidden rounded-[1.5rem] border border-white/10 bg-white/[0.045] shadow-2xl sm:rounded-[1.75rem]"><div className="h-1" style={{ backgroundColor: primary }} /><div className="p-4 sm:p-6 md:p-7"><p className="text-[9px] font-black uppercase tracking-[0.2em] text-white/40 sm:text-xs sm:tracking-[0.28em]">Stats On File</p><h2 className="mt-1.5 text-2xl font-black sm:mt-2 sm:text-3xl">Current VarsityVue rankings</h2><div className="mt-4 grid grid-cols-[repeat(auto-fit,minmax(150px,1fr))] gap-2 sm:mt-6 sm:gap-4">{hasRushing ? <><RankCard label="Overall Rushing" rank={rushingRank} /><RankCard label="District Rushing" rank={districtRushingRank} /></> : null}{hasPassing ? <RankCard label="Overall Passing" rank={passingRank} /> : null}{hasReceiving ? <RankCard label="Overall Receiving" rank={receivingRank} /> : null}</div><p className="mt-4 text-[10px] leading-4 text-white/35 sm:mt-5 sm:text-xs sm:leading-5">Rankings reflect only verified statistics currently on file with VarsityVue. Coverage is still expanding and may not include every player, school, or game in the coverage area.</p></div></section>}
        ...
        <section className="overflow-hidden rounded-[1.5rem] border border-white/10 bg-white/[0.045] shadow-2xl sm:rounded-[1.75rem]"><div className="p-4 sm:p-6 md:p-7"><div className="flex items-end justify-between gap-3"><div><p className="text-[9px] font-black uppercase tracking-[0.2em] text-white/40 sm:text-xs sm:tracking-[0.28em]">Weekly Game Log</p><h2 className="mt-1.5 text-2xl font-black sm:mt-2 sm:text-3xl">{SEASON} performances</h2></div>{school && <Link href={`/schools/${school.slug}`} className="shrink-0 text-[9px] font-black uppercase tracking-[0.1em] text-white/50 transition hover:text-white sm:text-xs sm:tracking-[0.14em]">{school.name} Hub →</Link>}</div>
          {gameLog.length > 0 ? <>
            <div className="mt-4 space-y-2.5 md:hidden sm:mt-6 sm:space-y-3">
              {gameLog.map((entry) => (
                <Link key={entry.gameId} href={`/games/${entry.gameId}`} className="block rounded-xl border border-white/10 bg-black/30 p-3.5 transition hover:bg-white/[0.05] sm:rounded-2xl sm:p-4">
                  ...
                  <div className="mt-3 grid grid-cols-[repeat(auto-fit,minmax(90px,1fr))] gap-1.5 sm:mt-4 sm:gap-2">
                    {hasRushing ? <GameLogStat label="Rush" value={formatRushing(entry.rushing)} completeness={entry.completeness.rushing} hasValues={Boolean(entry.rushing)} /> : null}
                    {hasPassing ? <GameLogStat label="Pass" value={formatPassing(entry.passing)} completeness={entry.completeness.passing} hasValues={Boolean(entry.passing)} /> : null}
                    {hasReceiving ? <GameLogStat label="Rec" value={formatReceiving(entry.receiving)} completeness={entry.completeness.receiving} hasValues={Boolean(entry.receiving)} /> : null}
                  </div>
            ...
            <div className="mt-6 hidden overflow-x-auto rounded-2xl border border-white/10 bg-black/30 md:block"><table className="w-full text-sm"><thead className="border-b border-white/10 text-[10px] font-black uppercase tracking-[0.12em] text-white/35"><tr><th className="px-4 py-3 text-left">Week</th><th className="px-4 py-3 text-left">Opponent</th><th className="px-4 py-3 text-left">Result</th>{hasRushing ? <th className="px-4 py-3 text-left">Rush</th> : null}{hasPassing ? <th className="px-4 py-3 text-left">Pass</th> : null}{hasReceiving ? <th className="px-4 py-3 text-left">Rec</th> : null}</tr></thead><tbody>{gameLog.map((entry) => <tr key={entry.gameId} className="border-t border-white/5 first:border-0 transition hover:bg-white/[0.035]"><td className="px-4 py-4 font-black text-white/45">{entry.week ?? "—"}</td><td className="px-4 py-4"><Link href={`/games/${entry.gameId}`} className="font-black text-white/80 transition hover:text-white">{entry.opponent}</Link></td><td className="px-4 py-4 font-black text-white/65">{entry.result ?? "—"}</td>{hasRushing ? <td className="px-4 py-4 text-white/65"><LogTableValue value={formatRushing(entry.rushing)} completeness={entry.completeness.rushing} hasValues={Boolean(entry.rushing)} /></td> : null}{hasPassing ? <td className="px-4 py-4 text-white/65"><LogTableValue value={formatPassing(entry.passing)} completeness={entry.completeness.passing} hasValues={Boolean(entry.passing)} /></td> : null}{hasReceiving ? <td className="px-4 py-4 text-white/65"><LogTableValue value={formatReceiving(entry.receiving)} completeness={entry.completeness.receiving} hasValues={Boolean(entry.receiving)} /></td> : null}</tr>)}</tbody></table></div>
...
function StatCard({ label, headline, details, completeness }: { label: string; headline: string; details: string; completeness: StatCompletenessDetail }) { return <div className="rounded-[1.2rem] border border-white/10 bg-white/[0.045] p-4 shadow-xl sm:rounded-[1.5rem] sm:p-5"><div className="flex flex-wrap items-center justify-between gap-2"><p className="text-[9px] font-black uppercase tracking-[0.16em] text-white/40 sm:text-xs sm:tracking-[0.22em]">{label}</p>{showCompleteness(completeness) ? <StatCompletenessBadge completeness={completeness} hasValues compact /> : null}</div><p className="mt-2 text-2xl font-black text-white sm:mt-3 sm:text-3xl">{headline}</p><p className="mt-1.5 text-xs leading-5 text-white/45 sm:mt-2 sm:text-sm">{details}</p></div>; }
function RankCard({ label, rank }: { label: string; rank?: number }) { return <div className="rounded-xl border border-white/10 bg-black/30 p-3 sm:rounded-2xl sm:p-5"><p className="text-[8px] font-black uppercase tracking-[0.12em] text-white/35 sm:text-[10px] sm:tracking-[0.16em]">{label}</p><p className="mt-1.5 text-xl font-black sm:mt-2 sm:text-3xl">{rank ? `#${rank}` : "—"}</p></div>; }
