...
import { combineCategoryStates } from "@/lib/stat-completeness";
      ...
      <section className="rounded-[1.25rem] border border-white/10 bg-white/[0.045] p-4 shadow-2xl sm:rounded-[1.75rem] sm:p-7">
        ...
        <div className="mt-4 grid gap-2.5 sm:mt-6 sm:gap-5 xl:grid-cols-2">
          {stats.tables.map((table) => {
            ...
            const completeness = combineCategoryStates(schoolSlugs.map((schoolSlug) => table.completeness?.[schoolSlug] ?? { status: "unknown" }), table.rows.length > 0);
            return <div key={table.title} className="overflow-hidden rounded-xl border border-white/10 bg-black/30 sm:rounded-2xl"><div className="flex flex-wrap items-center justify-between gap-2 border-b border-white/10 px-3 py-2.5 text-xs font-black sm:px-4 sm:py-3 sm:text-sm"><span>{table.title}</span><StatCompletenessBadge completeness={completeness} hasValues={table.rows.length > 0} compact /></div>
