    ...
    <section className="relative overflow-hidden border-b border-white/10 text-white" style={{ background: `radial-gradient(circle at top left, ${primary}66 0%, transparent 32%), radial-gradient(circle at top right, ${secondary}22 0%, transparent 34%), linear-gradient(120deg, #050505 0%, #080808 52%, #000000 100%)` }}>
      ...
      <div className="relative mx-auto grid max-w-[1440px] gap-4 px-4 py-5 sm:gap-5 sm:px-6 sm:py-8 lg:min-h-[500px] lg:grid-cols-[1.15fr_0.85fr] lg:gap-8 lg:px-8">
        <div className="flex min-w-0 flex-col justify-between"><div className="min-w-0">
          ...
          <div className="mt-3 flex min-w-0 flex-wrap items-center gap-1.5 sm:mt-6 sm:gap-3"><HeroChip label="2026 Football" /></div>
          <div className="mt-4 flex min-w-0 items-center gap-3 sm:mt-8 sm:gap-6">
            ...
            <div className="min-w-0">
              ...
              <div className="mt-3 sm:mt-4"><SchoolFollowControl schoolName={school.name} schoolSlug={school.slug} isAuthenticated={isAuthenticated} isFollowing={isFollowing} finishFollowing={finishFollowing} initialMessage={followMessage} /></div>
              {(school.headCoach || school.stadium) && <div className="mt-3 flex flex-wrap items-center gap-x-2 gap-y-1 text-[9px] font-bold uppercase tracking-[0.1em] text-white/50 sm:text-xs sm:tracking-[0.12em]">
                {school.headCoach && <span>Coach {school.headCoach}</span>}
                {school.headCoach && school.stadium && <span aria-hidden="true" className="text-white/25">·</span>}
