...
function formatShortClassification(classification: UILClassification) { return `${classification.conference}${classification.division ? ` ${classification.division}` : ""}`; }
...
export default function SchoolHero({ school }: { school: School }) {
  ...
  const seasonRecord = getSchoolRecord(school.slug);
  ...
  return (
    <section className="relative overflow-hidden border-b border-white/10 text-white" style={{ background: `radial-gradient(circle at top left, ${primary}66 0%, transparent 32%), radial-gradient(circle at top right, ${secondary}22 0%, transparent 34%), linear-gradient(120deg, #050505 0%, #080808 52%, #000000 100%)` }}>
      ...
      <div className="absolute inset-0 bg-[linear-gradient(90deg,rgba(0,0,0,0.78),rgba(0,0,0,0.22))]" /><div className="absolute -right-24 top-16 h-80 w-80 rounded-full bg-white/10 blur-3xl" />
      <div className="relative mx-auto grid max-w-[1440px] gap-5 px-4 py-6 sm:px-6 sm:py-8 lg:min-h-[500px] lg:grid-cols-[1.15fr_0.85fr] lg:gap-8 lg:px-8">
...
function Stat({ value, label }: { value: string; label: string }) { return <div className="min-w-0 rounded-xl border border-white/10 bg-black/35 p-3 sm:rounded-2xl sm:p-4"><p className="truncate text-lg font-black text-white sm:text-xl">{value}</p><p className="mt-1 truncate text-[10px] uppercase tracking-[0.14em] text-white/35 sm:text-xs sm:tracking-[0.2em]">{label}</p></div>; }
