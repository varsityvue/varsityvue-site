        ...
        <div className="flex min-w-0 flex-col justify-between"><div className="min-w-0">
          ...
          <div className="mt-3 grid min-w-0 grid-cols-[auto_minmax(0,1fr)] items-center gap-x-3 gap-y-2 sm:mt-8 sm:gap-x-6 sm:gap-y-3">
            ...
            <div className="col-span-2 mt-1 sm:col-span-1 sm:col-start-2 sm:mt-0">
              ...
              {!isFollowing && <p className="mt-1.5 max-w-sm text-[9px] leading-4 text-white/50 sm:text-[10px]">Save {school.name} to your account, then choose verified final-score alerts.</p>}
            </div>
...
function Stat({ value, label, href, compact = false, ariaLabel }: { value: string; label: string; href?: string; compact?: boolean; ariaLabel?: string }) {
  const content = <><p className={`${compact ? "text-[13px] leading-4 sm:text-lg sm:leading-6" : "text-base sm:text-xl"} break-words font-black text-white`}>{value}</p><p className="mt-0.5 text-[9px] uppercase tracking-[0.1em] text-white/55 sm:mt-1 sm:text-xs sm:tracking-[0.16em]">{label}</p></>;
