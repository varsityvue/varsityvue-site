        ...
        {games.map((game, index) => (
          <fieldset key={game.id} disabled={game.locked || pending} className="min-w-0 rounded-[1.2rem] border border-white/10 bg-black/25 p-3.5 disabled:opacity-60 sm:rounded-[1.5rem] sm:p-5">
            ...
            <div className="mt-3 grid grid-cols-2 gap-2">
              <PickChoice name={`pick_${game.id}`} slug={game.awaySlug} team={game.awayName} label="Away" mark={game.awayMark} color={game.awayColor} logoUrl={game.awayLogoUrl} logoFilter={game.awayLogoFilter} checked={selections[game.id] === game.awaySlug} onSelect={() => setSelections((current) => ({ ...current, [game.id]: game.awaySlug }))} />
              <PickChoice name={`pick_${game.id}`} slug={game.homeSlug} team={game.homeName} label="Home" mark={game.homeMark} color={game.homeColor} logoUrl={game.homeLogoUrl} logoFilter={game.homeLogoFilter} checked={selections[game.id] === game.homeSlug} onSelect={() => setSelections((current) => ({ ...current, [game.id]: game.homeSlug }))} />
            </div>
          </fieldset>
        ))}
      </div>
...
function PickChoice({ name, slug, team, label, mark, color, logoUrl, logoFilter, checked, onSelect }: { name: string; slug: string; team: string; label: string; mark: string; color: string; logoUrl?: string; logoFilter?: string; checked: boolean; onSelect: () => void }) {
  return (
    <label className="group relative cursor-pointer">
      ...
      <span className="flex min-h-24 flex-col items-center justify-center rounded-xl border border-white/10 bg-white/[0.04] px-2 py-3 text-center transition group-hover:bg-white/[0.08] peer-checked:border-[var(--vv-accent)] peer-checked:bg-[var(--vv-primary)]/35 peer-checked:shadow-[inset_0_0_0_1px_rgba(242,184,75,0.16)] peer-focus-visible:ring-2 peer-focus-visible:ring-white/70">
        <TeamMark mark={mark} color={color} logoUrl={logoUrl} logoFilter={logoFilter}/><span className="mt-2 text-[8px] font-black uppercase tracking-[0.14em] text-white/35">{label}</span>
