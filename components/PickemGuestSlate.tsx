        ...
        {games.map((game, index) => (
          <fieldset key={game.id} disabled={game.locked} className="min-w-0 rounded-[1.2rem] border border-white/10 bg-black/25 p-3.5 disabled:opacity-60 sm:rounded-[1.5rem] sm:p-5">
            ...
            <div className="mt-3 grid grid-cols-2 gap-2">
              <GuestChoice gameId={game.gameId} slug={game.awaySlug} team={game.awayName} label="Away" mark={game.awayMark} color={game.awayColor} logoUrl={game.awayLogoUrl} logoScale={game.awayLogoScale} checked={selections[game.gameId] === game.awaySlug} onSelect={setSelections} />
              <GuestChoice gameId={game.gameId} slug={game.homeSlug} team={game.homeName} label="Home" mark={game.homeMark} color={game.homeColor} logoUrl={game.homeLogoUrl} logoScale={game.homeLogoScale} checked={selections[game.gameId] === game.homeSlug} onSelect={setSelections} />
            </div>
          </fieldset>
        ))}
      </div>
...
function GuestChoice({ gameId, slug, team, label, mark, color, logoUrl, logoScale, checked, onSelect }: { gameId: string; slug: string; team: string; label: string; mark: string; color: string; logoUrl?: string; logoScale: number; checked: boolean; onSelect: Dispatch<SetStateAction<Record<string, string>>> }) {
  return <label className="group relative cursor-pointer"><input type="radio" name={`guest_${gameId}`} value={slug} checked={checked} onChange={() => onSelect((current) => ({ ...current, [gameId]: slug }))} className="peer sr-only" /><span className="flex min-h-24 flex-col items-center justify-center rounded-xl border border-white/10 bg-white/[0.04] px-2 py-3 text-center transition group-hover:bg-white/[0.08] peer-checked:border-[var(--vv-accent)] peer-checked:bg-[var(--vv-primary)]/35 peer-checked:shadow-[inset_0_0_0_1px_rgba(242,184,75,0.16)] peer-focus-visible:ring-2 peer-focus-visible:ring-white/70"><TeamMark mark={mark} color={color} logoUrl={logoUrl} logoScale={logoScale}/><span className="mt-2 text-[8px] font-black uppercase tracking-[0.14em] text-white/35">{label}</span><span className="mt-0.5 text-sm font-black leading-tight text-white sm:text-base">{team}</span>{checked ? <span className="mt-1 text-[8px] font-black uppercase tracking-[0.12em] text-[var(--vv-accent)]">Selected</span> : null}</span></label>;
}
